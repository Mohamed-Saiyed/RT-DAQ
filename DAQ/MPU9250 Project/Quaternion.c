	...
	if(filter_sel != 50)
	{
        ...
        switch (filter_sel)
		{
            ...
            case MAHONY:
                ...
                break;
            default:
                mahony_updateIMU(ax, ay, az, gx, gy, gz, q);
                break;
		}
	}
	else
...
void Mahony(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz, float* q)
{
	...
	// Normalise quaternion
	recipNorm = Mahony_invSqrt(q[0] * q[0] + q[1] * q[1] + q[2] * q[2] + q[3] * q[3]);
	q[0] *= recipNorm;
	q[1] *= recipNorm;
	q[2] *= recipNorm;
	q[3] *= recipNorm;
