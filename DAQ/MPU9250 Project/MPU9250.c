...
static  uint8_t MPU9250_DEFAULT_ADDRESS = 0x68;  // Device address when ADO = 0
static  uint8_t AK8963_ADDRESS = 0x18;           //  Address of magnetometer
...
static float mag_bias[3] = {95.9609299, -141.692307, 1151.53113};  // mag calibration value in MAG_OUTPUT_BITS: 16BITS
static float mag_scale[3] = {89.3333359, 0.350326806, 7.4444466};
...
static float q[4] = {1, 0, 0, 0};  // vector to hold quaternion
				  ...
				  FIFO_SAMPLE_RATE Rate,GYRO_DLPF_CFG GCfg,ACCEL_DLPF_CFG ACfg)
{
	...
	HAL_I2C_Mem_Read(GlobalConfig , MPU9250_ADDRESS , WHO_AM_I_MPU9250 , 1 , &Data , 1 , HAL_MAX_DELAY);
	if(Data == 0x71)
	{
		/*reset device*/
		Data = 0x80 ;		
		...
		HAL_I2C_Mem_Read(GlobalConfig , MPU9250_ADDRESS , ACCEL_CONFIG , 1 , &c , 1 , HAL_MAX_DELAY);
			...
			//delay(100);
			Data = 0x22;
				HAL_I2C_Mem_Write(GlobalConfig , MPU9250_ADDRESS , INT_PIN_CFG , 1 , &Data , 1 , HAL_MAX_DELAY);
				Data = 0x01;
					HAL_I2C_Mem_Write(GlobalConfig , MPU9250_ADDRESS , INT_ENABLE , 1 , &Data , 1 , HAL_MAX_DELAY);
	}
	else
	{
...
void MPU9250_MagnetometerInit(MAG_OUTPUT_BITS MagBits)
{
	...
	HAL_I2C_Mem_Read(GlobalConfig , AK8963_ADDRESS , 0x00 , 1 , &Data , 1 , HAL_MAX_DELAY);
	if(Data == 0x48)
	{
...
void MPU9250_MainUpdateFunction(void)
{
	...
	QuaternionUpdate(an, ae, ad, gn, ge, gd, mn, me, md, q);

	MPU9250UpdateEuler(q[0], q[1], q[2], q[3]);
	
	
}
void MPU9250UpdateEuler(float qw, float qx, float qy, float qz)
{
...
static void write_gyro_offset(void)
{
    ...
    HAL_I2C_Mem_Write(GlobalConfig , MPU9250_ADDRESS, XG_OFFSET_H, 1 , &gyro_offset_data[0], 1 , HAL_MAX_DELAY);
    HAL_I2C_Mem_Write(GlobalConfig , MPU9250_ADDRESS, XG_OFFSET_L, 1 , &gyro_offset_data[1], 1 , HAL_MAX_DELAY);
    HAL_I2C_Mem_Write(GlobalConfig , MPU9250_ADDRESS, YG_OFFSET_H, 1 , &gyro_offset_data[2], 1 , HAL_MAX_DELAY);
    HAL_I2C_Mem_Write(GlobalConfig , MPU9250_ADDRESS, YG_OFFSET_L, 1 , &gyro_offset_data[3], 1 , HAL_MAX_DELAY);
    HAL_I2C_Mem_Write(GlobalConfig , MPU9250_ADDRESS, ZG_OFFSET_H, 1 , &gyro_offset_data[4], 1 , HAL_MAX_DELAY);
    HAL_I2C_Mem_Write(GlobalConfig , MPU9250_ADDRESS, ZG_OFFSET_L, 1 , &gyro_offset_data[5], 1 , HAL_MAX_DELAY);
...
static void collect_mag_data_to(float* m_bias, float* m_scale) 
{
	...
	float bias_resolution = get_mag_resolution(M16BITS);
...
void MPU9250_UpdateAccelGyro(void)
{
	...
	MPU9250_Read_Accel_Gyro(raw_acc_gyro_data);  // INT cleared on any read
